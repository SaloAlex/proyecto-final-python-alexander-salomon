...
def listaVeterinarios(request):
    ...
    return render(request, 'AppCoder/lista_veterinarios.html', {'veterinarios': veterinarios})

def borrarVeterinario(request, veterinario_id):
    veterinario = get_object_or_404(Veterinario, id=veterinario_id)
    veterinario.delete()
    return redirect('listaVeterinarios')

def actualizarVeterinario(request, veterinario_id):
    veterinario = get_object_or_404(Veterinario, id=veterinario_id)
    ...
    return render(request, 'AppCoder/actualizar_veterinario.html', {'formulario': formulario})
...
#-----------------------------------------Avatar-------------------------------------------------------@login_required
def inicio(request):
    avatares = Avatar.objects.filter(user=request.user.id)
    print("Número de avatares:", len(avatares))

    if avatares:
        avatar_url = avatares[0].imagen.url
    else:
        # Proporciona una URL de imagen predeterminada o un valor nulo según tus necesidades.
        avatar_url = "/ruta/a/imagen/por/defecto.png"

    return render(request, "AppCoder/inicio.html", {"url": avatar_url})
